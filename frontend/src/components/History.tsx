    ...
    <div className="min-w-0 space-y-4 sm:space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex min-w-0 flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4">
        <div className="min-w-0">
          <h2 className="text-2xl sm:text-3xl font-bold tracking-tight">生成记录</h2>
          <p className="text-sm sm:text-base text-muted-foreground mt-1">本地保存的兑换码生成历史</p>
        </div>
        {historyItems.length > 0 && (
          <Button variant="destructive" size="sm" onClick={() => setClearDialog(true)} className="w-full sm:w-auto">
      ...
      <Card className="min-w-0">
        ...
        <CardContent className="space-y-4 px-3 pb-3 sm:px-6 sm:pb-6">
          {loading ? (
            ...
            <div className="text-center py-10 sm:py-12 bg-muted/20 rounded-lg border border-dashed px-4">
            ...
            <div className="min-w-0 space-y-3">
              {historyItems.map((item) => (
                <div key={item.id} className="min-w-0 border rounded-lg overflow-hidden transition-all duration-200 hover:shadow-md bg-card">
                  <div
                    className="flex min-w-0 items-center justify-between gap-2 p-3 sm:p-4 cursor-pointer hover:bg-muted/50 transition-colors"
                    onClick={() => setExpandedId(expandedId === item.id ? null : item.id)}
                  >
                    <div className="flex-1 min-w-0">
                      <div className="flex min-w-0 items-center gap-2 sm:gap-3">
                        <h3 className="text-sm font-medium truncate">{item.name}</h3>
                        <Badge variant="secondary" className="shrink-0 text-xs font-normal">{item.count} 个</Badge>
                      </div>
                      <div className="mt-1.5 flex min-w-0 flex-wrap items-center gap-x-3 sm:gap-x-4 gap-y-1 text-xs text-muted-foreground">
                  ...
                  {expandedId === item.id && (
                    <div className="p-3 sm:p-4 border-t bg-muted/30 space-y-4 animate-in slide-in-from-top-2 duration-200">
                      <div className="min-w-0">
                        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between mb-3">
                          <label className="text-sm font-medium text-muted-foreground">兑换码列表</label>
                          <div className="grid grid-cols-2 gap-2 sm:hidden">
                            <Button variant="outline" size="sm" className="h-8 text-xs" onClick={() => handleCopyKeys(item.keys)}>复制</Button>
                            <Button variant="outline" size="sm" className="h-8 text-xs" onClick={() => handleDownloadKeys(item)}>下载</Button>
                          </div>
                        </div>
                        <div className="bg-background border rounded-lg p-2 sm:p-3 max-h-48 overflow-y-auto space-y-1 custom-scrollbar">
                          {item.keys.slice(0, 20).map((key, index) => (
                            <div key={index} className="flex min-w-0 items-center justify-between gap-2 py-1.5 px-2 hover:bg-muted rounded group transition-colors">
                              <code className="min-w-0 flex-1 truncate text-xs font-mono text-foreground/80" title={key}>{key}</code>
                              <button 
                                onClick={() => copyToClipboard(key, '兑换码')} 
                                className="shrink-0 text-muted-foreground hover:text-primary transition-opacity p-1 sm:opacity-0 sm:group-hover:opacity-100"
                                title="复制单个"
                              >
                        ...
                        <div className="flex sm:hidden justify-end mt-4 pt-2 border-t">
                           <Button variant="ghost" size="sm" className="w-full text-destructive hover:text-destructive hover:bg-destructive/10 h-8" onClick={() => handleDelete(item.id)}>
